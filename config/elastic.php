    ...
    */
    'models' => [
        ...
        'App\\Models\\Product\\Product' => [
            ...
            // Boost значения для полей поиска - определяют приоритет полей в поиске
            // Структура аналогична searchable_fields, но с числовыми значениями boost
            // Если поле не указано или конфиг отсутствует, используется boost = 1.0
            'searchable_fields_boost' => [
                // Поля текущей модели с boost значениями
                'title' => 3.0,                    // Высокий приоритет для заголовка
                'slug' => 2.5,                     // Высокий приоритет для slug
                'short_description' => 2.0,        // Средний приоритет для краткого описания
                'specification' => 1.5,            // Средний приоритет для спецификации
                'description' => 1.0,              // Обычный приоритет для описания
                
                // Поля из relations с boost значениями
                'category' => [
                    'title' => 2.0,               // Средний приоритет для заголовка категории
                    'slug' => 1.5,                // Средний приоритет для slug категории
                    'manufacturer' => [
                        'name' => 1.5,            // Средний приоритет для названия производителя
                        'code' => 1.0             // Обычный приоритет для кода производителя
                    ]
                ],
                'brand' => [
                    'name' => 1.5,                // Средний приоритет для названия бренда
                    'slug' => 1.0                 // Обычный приоритет для slug бренда
                ],
            ],
            
            // Поля для возврата из базы данных после поиска
            // Определяет, какие поля и отношения загружать из БД
            // Формат: ['поле', 'отношение' => ['поле', 'вложенное_отношение' => ['поле']]]
            'return_fields' => [
                'id',                           // ID записи
                'title',                        // Заголовок
                'slug',                         // Slug
                ...
                'category' => [                 // Отношение категории
                    'id', 'name', 'slug', 'is_active'
                ],
                'brand' => [                    // Отношение бренда
                    'id', 'name', 'slug', 'logo'
                ],
                'images' => [                   // Отношение изображений
